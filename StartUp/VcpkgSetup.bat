...
ECHO Update de rapidjson...
CALL "%LOCAL_VCPKG_DIR%\vcpkg.exe" remove rapidjson:x64-windows
CALL "%LOCAL_VCPKG_DIR%\vcpkg.exe" install rapidjson:x64-windows
IF %ERRORLEVEL% NEQ 0 (
    ECHO Erreur lors de la mise à jour de rapidjson.
    PAUSE
    EXIT /B 1
)
...
ECHO Installation de rapidjson...
CALL "%LOCAL_VCPKG_DIR%\vcpkg.exe" install rapidjson
IF %ERRORLEVEL% NEQ 0 (
    ECHO Erreur lors de l'installation de rapidjson.
    PAUSE
    EXIT /B 1
)
